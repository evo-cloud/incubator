...
var Builder = Class(process.EventEmitter, {
    ...
    run: function (pkgNames, callback) {
        callback && this.once('complete', callback);
    ...
    build: function (callback) {
        ...
        this.packages.order().each(function (pkg, next) {
            workload.push(function (workerId, done) {
                BuildEngine.build(pkg, {
                        srcdir: self.cache.basedir(pkg),
                        basedir: self.cfg.buildDir,
                        outdir: self.cfg.relDir,
                        worker: workerId
                    })
                    ...
                    .build({ cleanBuild: self.cfg.cleanBuild, saveSpace: self.cfg.saveSpace }, function (err) {
